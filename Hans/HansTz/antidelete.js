    ...
    if (cmd === 'antidelete') {
        ...
        if (action === 'off') {
            if (antideleteSettings[m.from]) {
                ...
                fs.writeFileSync(AntideleteDB, JSON.stringify(antideleteSettings, null, 2));
                await sock.sendMessage(m.from, {
                    text: `━━❌ *Hans Xmd AntiDelete Deactivated* ❌━━`,
                    contextInfo: {
                        forwardingScore: 5,
                        isForwarded: true,
                        forwardedNewsletterMessageInfo: {
        ...
        if (action === 'set') {
            ...
            return await sock.sendMessage(m.from, {
                text: `━━✅ *Hans Xmd AntiDelete mode set to:* ${mode.toUpperCase()} ✅━━`,
                contextInfo: {
                    forwardingScore: 5,
                    isForwarded: true,
                    forwardedNewsletterMessageInfo: {
